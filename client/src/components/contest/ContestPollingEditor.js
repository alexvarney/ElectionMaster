...
function ContestPollingEditor (props) {
  ...
  const getPollingValue = candidateId => {
    const pollResult =
      selectedPoll && selectedPoll.values
        ? selectedPoll.values.filter(item => item.candidate === candidateId)[0]
        : null
  ...
  const setPollingValue = event => {
    ...
    const newList = selectedPoll.values.filter(
      item => item.candidate !== event.target.id
    )
  ...
  const updatePoll = event => {
    ...
    const newList = selectedContest.polling.filter(
      item =>
        moment(item.date).format('YYYY-MM-DD') !==
        moment(selectedPoll.date).format('YYYY-MM-DD')
    )
  ...
  const getDateSortedPolls = () => {
    return selectedContest.polling.sort((a, b) =>
      moment(a.date).format('YYYY-MM-DD') < moment(b.date).format('YYYY-MM-DD')
        ? 1
        : -1
    )
  }
  ...
  return (
    <Form>
      <FormGroup row>
        <Col>
          <InputGroup>
            ...
            <Input
              disabled={!selectedContest || !selectedPoll.date}
              value={selectedPoll.date ? selectedPoll.date : ''}
          ...
          <Table>
            ...
            <tbody>
              {selectedContest
                ? getPollingCandidates().map(candidate => (
                  <tr key={candidate._id}>
                    <th>{candidate.name}</th>
                    <td>
                      <Input
                        disabled={!selectedPoll.date}
                        id={candidate._id}
                        value={getPollingValue(candidate._id)}
                        onChange={setPollingValue}
                        type='number'
                      />
                    </td>
                  </tr>
        ...
        <Col>
          <ul className={styles.linkedCandidateList}>
            {selectedContest
              ? getDateSortedPolls().map(item => (
                <li onClick={() => setSelectedPoll(item)} key={item.date}>
                  {moment(item.date).format('YYYY-MM-DD')}
                </li>
              ))
              : null}
      ...
      <FormGroup className={styles.buttonRow} row>
        ...
        <Button color='danger' onClick={deletePoll} disabled={!selectedContest}>
          Delete Poll
        </Button>
        <Button
          disabled={!selectedContest}
          onClick={() => setSelectedPoll(newPoll)}
        >
          New Poll
        </Button>
      </FormGroup>
